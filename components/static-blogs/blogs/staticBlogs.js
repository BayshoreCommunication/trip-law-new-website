import {
  RAID_RIGHTS_ARTICLE_DESCRIPTION,
  RAID_RIGHTS_ARTICLE_TITLE,
  RAID_RIGHTS_FEATURE_IMAGE,
  RAID_RIGHTS_FEATURE_IMAGE_ALT,
  RAID_RIGHTS_FEATURE_IMAGE_CAPTION,
  RAID_RIGHTS_FEATURE_IMAGE_DESCRIPTION,
  RAID_RIGHTS_FEATURE_IMAGE_TITLE,
  RAID_RIGHTS_SLUG,
} from "./whatAreYourRightsDuringAnImmigrationRaidOrArrestMeta";
import {
  VISA_OVERSTAY_DESCRIPTION,
  VISA_OVERSTAY_FEATURE_IMAGE,
  VISA_OVERSTAY_FEATURE_IMAGE_ALT,
  VISA_OVERSTAY_TITLE,
} from "./visaOverstayMeta";
import {
  DENIED_APPLICATIONS_ARTICLE_DESCRIPTION,
  DENIED_APPLICATIONS_ARTICLE_TITLE,
  DENIED_APPLICATIONS_FEATURE_IMAGE,
  DENIED_APPLICATIONS_FEATURE_IMAGE_ALT,
  DENIED_APPLICATIONS_FEATURE_IMAGE_CAPTION,
  DENIED_APPLICATIONS_FEATURE_IMAGE_DESCRIPTION,
  DENIED_APPLICATIONS_FEATURE_IMAGE_TITLE,
  DENIED_APPLICATIONS_SLUG,
} from "./commonReasonsImmigrationApplicationsGetDeniedMeta";
import {
  IMMIGRATION_HOLDS_ARTICLE_DESCRIPTION,
  IMMIGRATION_HOLDS_ARTICLE_TITLE,
  IMMIGRATION_HOLDS_FEATURE_IMAGE,
  IMMIGRATION_HOLDS_FEATURE_IMAGE_ALT,
  IMMIGRATION_HOLDS_FEATURE_IMAGE_CAPTION,
  IMMIGRATION_HOLDS_FEATURE_IMAGE_DESCRIPTION,
  IMMIGRATION_HOLDS_FEATURE_IMAGE_TITLE,
  IMMIGRATION_HOLDS_SLUG,
} from "./immigrationHoldsIceDetainersMeta";
import {
  ENTREPRENEUR_ARTICLE_DESCRIPTION,
  ENTREPRENEUR_ARTICLE_TITLE,
  ENTREPRENEUR_FEATURE_IMAGE,
  ENTREPRENEUR_FEATURE_IMAGE_ALT,
  ENTREPRENEUR_FEATURE_IMAGE_CAPTION,
  ENTREPRENEUR_FEATURE_IMAGE_DESCRIPTION,
  ENTREPRENEUR_FEATURE_IMAGE_TITLE,
  ENTREPRENEUR_SLUG,
} from "./entrepreneurVisaMeta";
import {
  CRIMINAL_CHARGES_ARTICLE_DESCRIPTION,
  CRIMINAL_CHARGES_ARTICLE_TITLE,
  CRIMINAL_CHARGES_FEATURE_IMAGE,
  CRIMINAL_CHARGES_FEATURE_IMAGE_ALT,
  CRIMINAL_CHARGES_FEATURE_IMAGE_CAPTION,
  CRIMINAL_CHARGES_FEATURE_IMAGE_DESCRIPTION,
  CRIMINAL_CHARGES_FEATURE_IMAGE_TITLE,
  CRIMINAL_CHARGES_SLUG,
} from "./criminalChargesMeta";
import {
  ADMIN_PROCESSING_ARTICLE_DESCRIPTION,
  ADMIN_PROCESSING_ARTICLE_TITLE,
  ADMIN_PROCESSING_FEATURE_IMAGE,
  ADMIN_PROCESSING_FEATURE_IMAGE_ALT,
  ADMIN_PROCESSING_FEATURE_IMAGE_CAPTION,
  ADMIN_PROCESSING_FEATURE_IMAGE_DESCRIPTION,
  ADMIN_PROCESSING_FEATURE_IMAGE_TITLE,
  ADMIN_PROCESSING_SLUG,
} from "./administrativeProcessingMeta";
import {
  USCIS_INTERVIEW_ARTICLE_DESCRIPTION,
  USCIS_INTERVIEW_ARTICLE_TITLE,
  USCIS_INTERVIEW_FEATURE_IMAGE,
  USCIS_INTERVIEW_FEATURE_IMAGE_ALT,
  USCIS_INTERVIEW_FEATURE_IMAGE_CAPTION,
  USCIS_INTERVIEW_FEATURE_IMAGE_DESCRIPTION,
  USCIS_INTERVIEW_FEATURE_IMAGE_TITLE,
  USCIS_INTERVIEW_SLUG,
} from "./uscisInterviewMeta";
import {
  TRAVEL_RESTRICTIONS_ARTICLE_DESCRIPTION,
  TRAVEL_RESTRICTIONS_ARTICLE_TITLE,
  TRAVEL_RESTRICTIONS_FEATURE_IMAGE,
  TRAVEL_RESTRICTIONS_FEATURE_IMAGE_ALT,
  TRAVEL_RESTRICTIONS_FEATURE_IMAGE_CAPTION,
  TRAVEL_RESTRICTIONS_FEATURE_IMAGE_DESCRIPTION,
  TRAVEL_RESTRICTIONS_FEATURE_IMAGE_TITLE,
  TRAVEL_RESTRICTIONS_SLUG,
} from "./travelRestrictionsMeta";
import {
  PENDING_CASE_WORK_ARTICLE_DESCRIPTION,
  PENDING_CASE_WORK_ARTICLE_TITLE,
  PENDING_CASE_WORK_FEATURE_IMAGE,
  PENDING_CASE_WORK_FEATURE_IMAGE_ALT,
  PENDING_CASE_WORK_FEATURE_IMAGE_CAPTION,
  PENDING_CASE_WORK_FEATURE_IMAGE_DESCRIPTION,
  PENDING_CASE_WORK_FEATURE_IMAGE_TITLE,
  PENDING_CASE_WORK_SLUG,
} from "./whileYourImmigrationCaseIsPendingMeta";
import {
  WAIVER_GUIDE_ARTICLE_DESCRIPTION,
  WAIVER_GUIDE_ARTICLE_TITLE,
  WAIVER_GUIDE_FEATURE_IMAGE,
  WAIVER_GUIDE_FEATURE_IMAGE_ALT,
  WAIVER_GUIDE_FEATURE_IMAGE_CAPTION,
  WAIVER_GUIDE_FEATURE_IMAGE_DESCRIPTION,
  WAIVER_GUIDE_FEATURE_IMAGE_TITLE,
  WAIVER_GUIDE_SLUG,
} from "./waiverOfInadmissibilityMeta";
import {
  DIVORCE_EFFECTS_ARTICLE_DESCRIPTION,
  DIVORCE_EFFECTS_ARTICLE_TITLE,
  DIVORCE_EFFECTS_FEATURE_IMAGE,
  DIVORCE_EFFECTS_FEATURE_IMAGE_ALT,
  DIVORCE_EFFECTS_FEATURE_IMAGE_CAPTION,
  DIVORCE_EFFECTS_FEATURE_IMAGE_DESCRIPTION,
  DIVORCE_EFFECTS_FEATURE_IMAGE_TITLE,
  DIVORCE_EFFECTS_SLUG,
} from "./divorceEffectsMeta";
import {
  SPONSOR_MARRIED_CHILD_ARTICLE_DESCRIPTION,
  SPONSOR_MARRIED_CHILD_ARTICLE_TITLE,
  SPONSOR_MARRIED_CHILD_FEATURE_IMAGE,
  SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_ALT,
  SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_CAPTION,
  SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_DESCRIPTION,
  SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_TITLE,
  SPONSOR_MARRIED_CHILD_SLUG,
} from "./canUsCitizenSponsorMarriedSonOrDaughterMeta";
import {
  PETITIONER_DIES_ARTICLE_DESCRIPTION,
  PETITIONER_DIES_ARTICLE_TITLE,
  PETITIONER_DIES_FEATURE_IMAGE,
  PETITIONER_DIES_FEATURE_IMAGE_ALT,
  PETITIONER_DIES_FEATURE_IMAGE_CAPTION,
  PETITIONER_DIES_FEATURE_IMAGE_DESCRIPTION,
  PETITIONER_DIES_FEATURE_IMAGE_TITLE,
  PETITIONER_DIES_SLUG,
} from "./petitionerDiesWhileGreenCardCasePendingMeta";
import {
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_ARTICLE_DESCRIPTION,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_ARTICLE_TITLE,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_ALT,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_CAPTION,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_DESCRIPTION,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_TITLE,
  CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_SLUG,
} from "./canGreenCardHolderSponsorSiblingsGuideMeta";
import {
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_ARTICLE_DESCRIPTION,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_ARTICLE_TITLE,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_ALT,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_CAPTION,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_DESCRIPTION,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_TITLE,
  CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_SLUG,
} from "./canYouChangeYourNameDuringNaturalizationMeta";
import {
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_ARTICLE_DESCRIPTION,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_ARTICLE_TITLE,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_ALT,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_CAPTION,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_DESCRIPTION,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_TITLE,
  AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_SLUG,
} from "./ageOutProtectionUnderTheChildStatusProtectionActMeta";

export const staticBlogs = [
  {
    slug: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_SLUG,
    title: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_ARTICLE_TITLE,
    body: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_ARTICLE_DESCRIPTION,
    createdAt: "2026-09-14",
    published: true,
    featuredImage: {
      image: {
        url: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE,
      },
      altText: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_ALT,
      title: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_TITLE,
      description: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_DESCRIPTION,
      caption: AGE_OUT_PROTECTION_UNDER_THE_CHILD_STATUS_PROTECTION_ACT_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_SLUG,
    title: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_ARTICLE_TITLE,
    body: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_ARTICLE_DESCRIPTION,
    createdAt: "2026-09-07",
    published: true,
    featuredImage: {
      image: {
        url: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE,
      },
      altText: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_ALT,
      title: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_TITLE,
      description: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_DESCRIPTION,
      caption: CAN_YOU_CHANGE_YOUR_NAME_DURING_NATURALIZATION_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_SLUG,
    title: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_ARTICLE_TITLE,
    body: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_ARTICLE_DESCRIPTION,
    createdAt: "2026-08-30",
    published: true,
    featuredImage: {
      image: {
        url: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE,
      },
      altText: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_ALT,
      title: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_TITLE,
      description: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_DESCRIPTION,
      caption: CAN_GREEN_CARD_HOLDER_SPONSOR_SIBLINGS_GUIDE_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: PETITIONER_DIES_SLUG,
    title: PETITIONER_DIES_ARTICLE_TITLE,
    body: PETITIONER_DIES_ARTICLE_DESCRIPTION,
    createdAt: "2026-08-23",
    published: true,
    featuredImage: {
      image: {
        url: PETITIONER_DIES_FEATURE_IMAGE,
      },
      altText: PETITIONER_DIES_FEATURE_IMAGE_ALT,
      title: PETITIONER_DIES_FEATURE_IMAGE_TITLE,
      description: PETITIONER_DIES_FEATURE_IMAGE_DESCRIPTION,
      caption: PETITIONER_DIES_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: SPONSOR_MARRIED_CHILD_SLUG,
    title: SPONSOR_MARRIED_CHILD_ARTICLE_TITLE,
    body: SPONSOR_MARRIED_CHILD_ARTICLE_DESCRIPTION,
    createdAt: "2026-08-17",
    published: true,
    featuredImage: {
      image: {
        url: SPONSOR_MARRIED_CHILD_FEATURE_IMAGE,
      },
      altText: SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_ALT,
      title: SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_TITLE,
      description: SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_DESCRIPTION,
      caption: SPONSOR_MARRIED_CHILD_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: DIVORCE_EFFECTS_SLUG,
    title: DIVORCE_EFFECTS_ARTICLE_TITLE,
    body: DIVORCE_EFFECTS_ARTICLE_DESCRIPTION,
    createdAt: "2026-07-28",
    published: true,
    featuredImage: {
      image: {
        url: DIVORCE_EFFECTS_FEATURE_IMAGE,
      },
      altText: DIVORCE_EFFECTS_FEATURE_IMAGE_ALT,
      title: DIVORCE_EFFECTS_FEATURE_IMAGE_TITLE,
      description: DIVORCE_EFFECTS_FEATURE_IMAGE_DESCRIPTION,
      caption: DIVORCE_EFFECTS_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: RAID_RIGHTS_SLUG,
    title: RAID_RIGHTS_ARTICLE_TITLE,
    body: RAID_RIGHTS_ARTICLE_DESCRIPTION,
    createdAt: "2026-07-21",
    published: true,
    featuredImage: {
      image: {
        url: RAID_RIGHTS_FEATURE_IMAGE,
      },
      altText: RAID_RIGHTS_FEATURE_IMAGE_ALT,
      title: RAID_RIGHTS_FEATURE_IMAGE_TITLE,
      description: RAID_RIGHTS_FEATURE_IMAGE_DESCRIPTION,
      caption: RAID_RIGHTS_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: IMMIGRATION_HOLDS_SLUG,
    title: IMMIGRATION_HOLDS_ARTICLE_TITLE,
    body: IMMIGRATION_HOLDS_ARTICLE_DESCRIPTION,
    createdAt: "2026-07-14",
    published: true,
    featuredImage: {
      image: {
        url: IMMIGRATION_HOLDS_FEATURE_IMAGE,
      },
      altText: IMMIGRATION_HOLDS_FEATURE_IMAGE_ALT,
      title: IMMIGRATION_HOLDS_FEATURE_IMAGE_TITLE,
      description: IMMIGRATION_HOLDS_FEATURE_IMAGE_DESCRIPTION,
      caption: IMMIGRATION_HOLDS_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: DENIED_APPLICATIONS_SLUG,
    title: DENIED_APPLICATIONS_ARTICLE_TITLE,
    body: DENIED_APPLICATIONS_ARTICLE_DESCRIPTION,
    createdAt: "2026-07-08",
    published: true,
    featuredImage: {
      image: {
        url: DENIED_APPLICATIONS_FEATURE_IMAGE,
      },
      altText: DENIED_APPLICATIONS_FEATURE_IMAGE_ALT,
      title: DENIED_APPLICATIONS_FEATURE_IMAGE_TITLE,
      description: DENIED_APPLICATIONS_FEATURE_IMAGE_DESCRIPTION,
      caption: DENIED_APPLICATIONS_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: WAIVER_GUIDE_SLUG,
    title: WAIVER_GUIDE_ARTICLE_TITLE,
    body: WAIVER_GUIDE_ARTICLE_DESCRIPTION,
    createdAt: "2026-06-29",
    published: true,
    featuredImage: {
      image: {
        url: WAIVER_GUIDE_FEATURE_IMAGE,
      },
      altText: WAIVER_GUIDE_FEATURE_IMAGE_ALT,
      title: WAIVER_GUIDE_FEATURE_IMAGE_TITLE,
      description: WAIVER_GUIDE_FEATURE_IMAGE_DESCRIPTION,
      caption: WAIVER_GUIDE_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: PENDING_CASE_WORK_SLUG,
    title: PENDING_CASE_WORK_ARTICLE_TITLE,
    body: PENDING_CASE_WORK_ARTICLE_DESCRIPTION,
    createdAt: "2026-06-21",
    published: true,
    featuredImage: {
      image: {
        url: PENDING_CASE_WORK_FEATURE_IMAGE,
      },
      altText: PENDING_CASE_WORK_FEATURE_IMAGE_ALT,
      title: PENDING_CASE_WORK_FEATURE_IMAGE_TITLE,
      description: PENDING_CASE_WORK_FEATURE_IMAGE_DESCRIPTION,
      caption: PENDING_CASE_WORK_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: TRAVEL_RESTRICTIONS_SLUG,
    title: TRAVEL_RESTRICTIONS_ARTICLE_TITLE,
    body: TRAVEL_RESTRICTIONS_ARTICLE_DESCRIPTION,
    createdAt: "2026-06-15",
    published: true,
    featuredImage: {
      image: {
        url: TRAVEL_RESTRICTIONS_FEATURE_IMAGE,
      },
      altText: TRAVEL_RESTRICTIONS_FEATURE_IMAGE_ALT,
      title: TRAVEL_RESTRICTIONS_FEATURE_IMAGE_TITLE,
      description: TRAVEL_RESTRICTIONS_FEATURE_IMAGE_DESCRIPTION,
      caption: TRAVEL_RESTRICTIONS_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: USCIS_INTERVIEW_SLUG,
    title: USCIS_INTERVIEW_ARTICLE_TITLE,
    body: USCIS_INTERVIEW_ARTICLE_DESCRIPTION,
    createdAt: "2026-06-07",
    published: true,
    featuredImage: {
      image: {
        url: USCIS_INTERVIEW_FEATURE_IMAGE,
      },
      altText: USCIS_INTERVIEW_FEATURE_IMAGE_ALT,
      title: USCIS_INTERVIEW_FEATURE_IMAGE_TITLE,
      description: USCIS_INTERVIEW_FEATURE_IMAGE_DESCRIPTION,
      caption: USCIS_INTERVIEW_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: ADMIN_PROCESSING_SLUG,
    title: ADMIN_PROCESSING_ARTICLE_TITLE,
    body: ADMIN_PROCESSING_ARTICLE_DESCRIPTION,
    createdAt: "2026-05-25",
    published: true,
    featuredImage: {
      image: {
        url: ADMIN_PROCESSING_FEATURE_IMAGE,
      },
      altText: ADMIN_PROCESSING_FEATURE_IMAGE_ALT,
      title: ADMIN_PROCESSING_FEATURE_IMAGE_TITLE,
      description: ADMIN_PROCESSING_FEATURE_IMAGE_DESCRIPTION,
      caption: ADMIN_PROCESSING_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: CRIMINAL_CHARGES_SLUG,
    title: CRIMINAL_CHARGES_ARTICLE_TITLE,
    body: CRIMINAL_CHARGES_ARTICLE_DESCRIPTION,
    createdAt: "2026-05-23",
    published: true,
    featuredImage: {
      image: {
        url: CRIMINAL_CHARGES_FEATURE_IMAGE,
      },
      altText: CRIMINAL_CHARGES_FEATURE_IMAGE_ALT,
      title: CRIMINAL_CHARGES_FEATURE_IMAGE_TITLE,
      description: CRIMINAL_CHARGES_FEATURE_IMAGE_DESCRIPTION,
      caption: CRIMINAL_CHARGES_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: ENTREPRENEUR_SLUG,
    title: ENTREPRENEUR_ARTICLE_TITLE,
    body: ENTREPRENEUR_ARTICLE_DESCRIPTION,
    createdAt: "2026-05-14",
    published: true,
    featuredImage: {
      image: {
        url: ENTREPRENEUR_FEATURE_IMAGE,
      },
      altText: ENTREPRENEUR_FEATURE_IMAGE_ALT,
      title: ENTREPRENEUR_FEATURE_IMAGE_TITLE,
      description: ENTREPRENEUR_FEATURE_IMAGE_DESCRIPTION,
      caption: ENTREPRENEUR_FEATURE_IMAGE_CAPTION,
    },
  },
  {
    slug: "visa-overstay",
    title: VISA_OVERSTAY_TITLE,
    body: VISA_OVERSTAY_DESCRIPTION,
    createdAt: "2026-05-10",
    published: true,
    featuredImage: {
      image: {
        url: VISA_OVERSTAY_FEATURE_IMAGE,
      },
      altText: VISA_OVERSTAY_FEATURE_IMAGE_ALT,
    },
  },
];

export function getPublishedBlogsWithStatic(blogPostData) {
  const blogsBySlug = new Map();
  const publishedBlogs = [
    ...staticBlogs,
    ...(blogPostData?.data?.filter((blog) => blog.published === true) || []),
  ];

  publishedBlogs.forEach((blog) => {
    if (blog?.slug && !blogsBySlug.has(blog.slug)) {
      blogsBySlug.set(blog.slug, blog);
    }
  });

  return [...blogsBySlug.values()].sort(
    (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
  );
}
